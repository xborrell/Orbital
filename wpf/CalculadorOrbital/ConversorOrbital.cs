...
namespace CalculadorOrbital
{
    ...
    {
        public OrbitalElements Convertir(Vector3D posicion, Vector3D velocidad)
        {
            float vr = (float)Math.Round((posicion.X * velocidad.X + posicion.Y * velocidad.Y + posicion.Z * velocidad.Z) / posicion.Length, 4);
            Vector3D angularMomentum = Vector3D.CrossProduct(posicion, velocidad);
            float inclination = (float)Math.Acos(angularMomentum.Z / angularMomentum.Length);
            Vector3D nodeLine = Vector3D.CrossProduct(Constantes.Forward, angularMomentum);
            float angleOfAscendingNode = NormalizarCuadrante(nodeLine.Y < 0, Math.Acos(nodeLine.X / nodeLine.Length));
            Vector3D excentricityVector = (1 / Constantes.Mu) * ((posicion * (float)(Math.Pow(velocidad.Length, 2) - (Constantes.Mu / posicion.Length))) - (velocidad * posicion.Length * vr));
            double excentricity = excentricityVector.Length;
            float argumentOfPeriapsis = NormalizarCuadrante(excentricityVector.Z < 0, Math.Acos(Vector3D.DotProduct(nodeLine, excentricityVector) / (nodeLine.Length * excentricity)));
            float trueAnomaly = NormalizarCuadrante(vr < 0, Math.Acos(Vector3D.DotProduct(excentricityVector, posicion) / (excentricity * posicion.Length)));

            return new OrbitalElements((float)angularMomentum.Length, (float)excentricity, inclination, angleOfAscendingNode, argumentOfPeriapsis, trueAnomaly);
        }
