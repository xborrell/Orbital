...
{
    public class CalculadorRotacion2
    {
        protected SateliteData _data;

        public CalculadorRotacion2(SateliteData data)
        {
            _data = data;
        }
    ...
    public class CalculadorRotacion
    {
        ...
        public Quaternion CalcularNuevaRotacion(float deltaTime)
        {
            ...
            switch (_data.Actitud)
            {
                ...
                case ActitudRotacion.Maniobrando: maniobra.CalcularNuevaRotacion(deltaTime); break;
                ...
                default: throw new ArgumentException("Actitud no implementada en CalculadorRotacion");
            }
        ...
        {
            Vector3D forward = _data.Posicion;
            forward.Normalize();
            forward *= -1;

            Vector3D upward = _data.Velocidad;
            upward.Normalize();

            return QuaternionExtensions.LookRotation(forward, upward);
        }

        Quaternion CalcularRotacionOrbital()
        {

            Vector3D forward = _data.Velocidad;
            forward.Normalize();

            Vector3D upward = _data.Posicion;
            upward.Normalize();
